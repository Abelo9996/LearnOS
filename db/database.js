...
// The old in-app forum (threads, replies, votes, leaderboard) is gone. A
// single-user self-hosted app cannot host a community, and a forum where you
// are the only member was a fiction. The real thing lives on a registry server
// that LearnOS talks to as a client (routes/registry.js), alongside sharing
// courses as files (routes/share.js).
//
// Drop the leftover tables — but only when empty, so nobody who actually wrote
// something into an older install loses it to a cleanup.
for (const t of ['community_votes', 'community_replies', 'community_threads']) {
  try {
    const n = db.prepare(`SELECT COUNT(*) c FROM ${t}`).get().c;
    if (n === 0) db.exec(`DROP TABLE ${t}`);
    else console.log(`Kept legacy table ${t} — it still holds ${n} row(s).`);
  } catch { /* already absent */ }
}

// Course syllabus tables. Created empty — courses are generated by the learner,
