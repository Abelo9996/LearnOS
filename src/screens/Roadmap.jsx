...
export default function Roadmap({ onOpenSession, onOpenCourse }) {
  ...
  // One outcome: a course pathway. With a key we plan a full specialization
  // (which also writes a placement diagnostic); without one we fall back to the
  // Curriculum agent's pathway, whose offline template is still an ordered
  // sequence of courses — never a concept map.
  const pollJob = async (jobId, { tries, everyMs }) => {
  ...
  const handleGenerate = async ({ goal, level, hours }) => {
    ...
    try {
      ...
      try {
        ...
        const result = await pollJob(jobId, { tries: 120, everyMs: 1000 });
        if (!result?.roadmap_id) throw new Error('Pathway planning returned nothing');
        await loadAllRoadmaps();
        await loadRoadmap(result.roadmap_id);
        toast(
          `Pathway planned · ${result.courses} courses toward "${goal}"` +
          (result.diagnosticQuestions ? ' — take the placement diagnostic to skip what you already know' : ''),
          'success',
        );
        return;
      } catch (e) {
        // Only fall back for a missing/rejected key. A genuine planning failure
        // should surface, not be quietly downgraded.
        if (!/key|NO_KEY|401|402|403|credit|limit/i.test(e.message || '')) throw e;
      }

      const { jobId } = await API.genRoadmap(goal);
      const result = await pollJob(jobId, { tries: 40, everyMs: 700 });
      if (!result?.roadmapId) throw new Error('Generation returned nothing');
      await loadAllRoadmaps();
      await loadRoadmap(result.roadmapId);
      toast(
        result.source === 'ai'
          ? `Pathway generated by the Curriculum agent · ${result.nodeCount} courses`
          : `Pathway created from an offline template · ${result.nodeCount} courses — add an API key in Settings for one built around your goal`,
        result.source === 'ai' ? 'success' : 'info',
      );
    } catch (e) {
      toast(e.message || 'Could not generate the pathway', 'error');
    } finally {
      setGenerating(false);
    }
  };
...
function GenerateModal({ onGenerate, onCancel }) {
  ...
  // There is one kind of roadmap: a course pathway. The old "Concept map"
  // alternative produced a graph of topics to browse, which is not what this
  // product is for.
...
function ViewToggle({ view, setView }) {
  // No graph view: LearnOS roadmaps are course pathways, not concept maps.
  // "Pathway" is the ordered sequence; "Progress" groups the same stages by state.
  const opts = [{ id: 'list', label: 'Pathway' }, { id: 'kanban', label: 'Progress' }];
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {nodes.length > 0 && (
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>The new stage is placed after these in the pathway and unlocks once they are mastered.</div>
          </div>
        )}
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
          <Btn variant="outline" onClick={onCancel}>Cancel</Btn>
