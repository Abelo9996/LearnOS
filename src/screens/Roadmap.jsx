...
import { generatePathway, describePathway } from '../lib/generatePathway.js';
...
export default function Roadmap({ onOpenSession, onOpenCourse }) {
  ...
  const handleGenerate = async ({ goal, level, hours }) => {
    ...
    try {
      ...
      const result = await generatePathway({ goal, level, profile: { level, time_per_week: Number(hours) || 5 } });
      await loadAllRoadmaps();
      await loadRoadmap(result.roadmapId);
      const { tone, message } = describePathway(result, goal);
      toast(message, tone);
    } catch (e) {
    ...
    <PageScroll>
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {/* Stage titles are long enough to blow out the header, so the
                label is capped and the full name lives in the tooltip. */}
            <Btn variant="primary" size="md" icon={I.play} onClick={handleResume}
              title={sel?.title || 'Resume'}>
              {(() => {
                const verb = sel?.status === 'active' ? 'Resume' : sel?.status === 'next' ? 'Start' : null;
                if (sel?.status === 'locked') return 'Locked';
                if (!verb || !sel?.title) return 'Resume';
                const t = sel.title.length > 26 ? `${sel.title.slice(0, 25)}…` : sel.title;
                return `${verb} · ${t}`;
              })()}
            </Btn>
          </>
      ...
      <Card pad={false}>
        {nodes.length === 0 ? (
          ...
          <>
            {view === 'map'    && <RoadmapGraph  nodes={nodes} edges={r.edges || []} selected={selected} setSelected={setSelected} highlightedIds={highlightedIds} />}
...
function ViewToggle({ view, setView }) {
  ...
  const opts = [{ id: 'map', label: 'Map' }, { id: 'list', label: 'Pathway' }, { id: 'kanban', label: 'Progress' }];
  return (
