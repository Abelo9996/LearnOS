        ...
        <Btn variant="primary" size="md" icon={I.spark} onClick={async () => {
          ...
          try {
            ...
            await API.createAssignment({
              ...
              steps: a.steps, // interactive, auto-graded steps (Phase 1)
...
function AssignmentExam({ a, onClose }) {
  // Interactive, auto-graded assignments (Phase 1) render the stepper; older
  // essay-style assignments keep the legacy single-textarea flow below.
  if (Array.isArray(a.steps) && a.steps.length) return <StepAssignment a={a} onClose={onClose} />;
  const { add: toast } = useToast();
...
/* ─────────────────────────────────────────────────────────────────────────────
   Interactive, auto-graded assignments (Phase 1)
...
const STEP_TYPE_META = {
  mcq:  { label: 'Concept check', color: 'var(--brand-2)',        icon: 'check' },
  code: { label: 'Coding task',   color: 'oklch(0.76 0.16 145)',  icon: 'spark' },
  short:{ label: 'Short answer',  color: 'oklch(0.74 0.17 220)',  icon: 'book' },
  written:{ label: 'Written',     color: 'oklch(0.74 0.17 220)',  icon: 'book' },
};
const stepSleep = (ms) => new Promise(r => setTimeout(r, ms));

function StepAssignment({ a, onClose }) {
  const { add: toast } = useToast();
  const steps = a.steps;
  const storeKey = `learnos_stepasgn_${a.id}`;
  const [answers, setAnswers] = React.useState({});
  const [grading, setGrading] = React.useState(false);
  const [result, setResult]   = React.useState(null); // { grade, feedback_md, rubric, steps_result }
  const graded = !!result;

  React.useEffect(() => {
    let alive = true;
    let draft = {};
    try { draft = JSON.parse(localStorage.getItem(storeKey) || '{}'); } catch {}
    if (Object.keys(draft).length) setAnswers(draft);
    API.getAssignmentSubmission(a.id).then(res => {
      if (!alive || !res?.submission) return;
      const sub = res.submission;
      let saved = {}; try { saved = JSON.parse(sub.answers_json || '{}'); } catch {}
      if (Object.keys(saved).length && !Object.keys(draft).length) setAnswers(saved);
      if (res.status === 'graded' && sub.grade != null) {
        let sr = {}; try { sr = JSON.parse(sub.steps_result_json || '{}'); } catch {}
        let rubric = []; try { rubric = JSON.parse(sub.rubric_json || '[]'); } catch {}
        if (Object.keys(saved).length) setAnswers(saved);
        setResult({ grade: sub.grade, feedback_md: sub.feedback_md, rubric, steps_result: sr });
      }
    }).catch(() => {});
    return () => { alive = false; };
  }, [a.id]);

  React.useEffect(() => { if (!graded) { try { localStorage.setItem(storeKey, JSON.stringify(answers)); } catch {} } }, [answers, graded, storeKey]);

  const setAns = (id, patch) => setAnswers(prev => ({ ...prev, [id]: { ...(prev[id] || {}), ...patch } }));

  const isAttempted = (s) => {
    const ans = answers[s.id] || {};
    if (s.type === 'mcq') return Number.isInteger(ans.selected_idx);
    if (s.type === 'code') return (ans.code || '').trim().length > 0;
    return (ans.text || '').trim().length > 0;
  };
  const attempted = steps.filter(isAttempted).length;
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        const poll = await API.getAssignmentSubmission(a.id).catch(() => null);
        if (poll?.status === 'graded') { final = poll.submission; break; }
      }
      if (final) {
        let sr = {}; try { sr = JSON.parse(final.steps_result_json || '{}'); } catch {}
        let rubric = []; try { rubric = JSON.parse(final.rubric_json || '[]'); } catch {}
        setResult({ grade: final.grade, feedback_md: final.feedback_md, rubric, steps_result: sr });
  ...
  return (
    <PageScroll>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
              ...
              {a.course && <Tag>{a.course}</Tag>}
              <Tag tone="accent">{steps.length} interactive steps</Tag>
              {graded && <Tag tone="good">Graded</Tag>}
            </div>
            <h1 className="display" style={{ fontSize: 28, color: 'var(--ink)', margin: '2px 0 10px' }}>{a.title}</h1>
            <MarkdownText prose text={a.description || 'Work through each step below. Run your code, answer the checks, and submit for grading.'} />
          </div>
          {grade != null && <Ring value={grade / 100} size={78} sw={7} color={gradeColor} label={`${grade}%`} />}
        </div>
      </Card>
      ...
      {result?.feedback_md && (
        <Card style={{ padding: 20, marginBottom: 16, animation: 'pageEnter var(--dur-normal) var(--ease-out)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: gradeColor }}>{grade >= 85 ? 'Strong work' : grade >= 70 ? 'Passed' : 'Keep going'}</span>
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>· graded step-by-step by the Assessment agent</span>
          </div>
          <div style={{ fontSize: 13.5, lineHeight: 1.65 }}><MarkdownText text={result.feedback_md} /></div>
        </Card>
      )}

      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {steps.map((s, i) => (
          <StepCard key={s.id} a={a} step={s} idx={i} answer={answers[s.id] || {}} setAns={(patch) => setAns(s.id, patch)}
            disabled={graded || grading} graded={graded} stepResult={result?.steps_result?.[s.id]} />
        ))}
      </div>

      {!graded && (
        <div style={{ position: 'sticky', bottom: 0, marginTop: 18, padding: '14px 18px', borderRadius: 12, background: 'color-mix(in oklch, var(--surface) 92%, transparent)', backdropFilter: 'blur(8px)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <span className="cap">{attempted}/{steps.length} steps attempted</span>
              <span className="mono" style={{ fontSize: 11, color: 'var(--muted)' }}>{Math.round((attempted / steps.length) * 100)}%</span>
            </div>
            <ProgressBar value={attempted / steps.length} height={6} />
          </div>
          <Btn variant="primary" size="md" disabled={grading || attempted === 0} onClick={submit}>
...
// One step, rendered by type. Autosaves through setAns; shows its own graded
// result once the assignment has been submitted.
function StepCard({ a, step, idx, answer, setAns, disabled, graded, stepResult }) {
  const tm = STEP_TYPE_META[step.type] || STEP_TYPE_META.short;
  const badge = stepResult ? (stepResult.passed ? { t: `✓ ${stepResult.score}%`, c: 'var(--good)' } : stepResult.score >= 60 ? { t: `${stepResult.score}%`, c: 'var(--warn)' } : { t: `${stepResult.score}%`, c: 'var(--bad)' }) : null;
  return (
    <Card style={{ padding: 20, borderColor: badge ? `color-mix(in oklch, ${badge.c} 45%, var(--border))` : undefined }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <span style={{ width: 26, height: 26, borderRadius: 7, background: 'var(--surface)', border: '1px solid var(--border)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'var(--brand)' }}>{idx + 1}</span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 9px', borderRadius: 999, fontSize: 11, fontWeight: 600, background: `color-mix(in oklch, ${tm.color} 16%, transparent)`, color: tm.color, border: `1px solid color-mix(in oklch, ${tm.color} 35%, transparent)` }}>
          {React.cloneElement(I[tm.icon] || I.book, { size: 11 })} {tm.label}
        </span>
        {step.weight > 1 && <span className="mono" style={{ fontSize: 10.5, color: 'var(--muted)' }}>weight ×{step.weight}</span>}
        <span style={{ flex: 1 }} />
        {badge && <span style={{ fontSize: 12.5, fontWeight: 700, color: badge.c }}>{badge.t}</span>}
      </div>

      <div style={{ fontSize: 14.5, color: 'var(--ink)', lineHeight: 1.6, marginBottom: 14 }}><MarkdownText text={step.prompt} /></div>

      {step.type === 'mcq'  && <McqStepBody step={step} answer={answer} setAns={setAns} disabled={disabled} stepResult={stepResult} />}
      {step.type === 'code' && <CodeStepBody a={a} step={step} answer={answer} setAns={setAns} disabled={disabled} stepResult={stepResult} />}
      {(step.type === 'short' || step.type === 'written') && <ShortStepBody step={step} answer={answer} setAns={setAns} disabled={disabled} stepResult={stepResult} />}
    </Card>
  );
}

function McqStepBody({ step, answer, setAns, disabled, stepResult }) {
  const sel = Number.isInteger(answer.selected_idx) ? answer.selected_idx : -1;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {step.choices.map((c, i) => {
        const chosen = sel === i;
        const isCorrect = stepResult && i === stepResult.correctIdx;
        const isWrongPick = stepResult && chosen && i !== stepResult.correctIdx;
        const border = isCorrect ? 'var(--good)' : isWrongPick ? 'var(--bad)' : chosen ? 'var(--accent-line)' : 'var(--border)';
        const bg = isCorrect ? 'color-mix(in oklch, var(--good) 12%, transparent)' : isWrongPick ? 'color-mix(in oklch, var(--bad) 12%, transparent)' : chosen ? 'var(--accent-soft)' : 'var(--surface)';
        return (
          <label key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, background: bg, border: `1px solid ${border}`, cursor: disabled ? 'default' : 'pointer' }}>
            <input type="radio" name={`mcq-${step.id}`} checked={chosen} disabled={disabled} onChange={() => setAns({ selected_idx: i })} style={{ accentColor: 'var(--brand)' }} />
            <span style={{ fontSize: 13.5, color: 'var(--ink)', flex: 1 }}>{c}</span>
            {isCorrect && <span style={{ color: 'var(--good)', fontSize: 12, fontWeight: 700 }}>correct</span>}
            {isWrongPick && <span style={{ color: 'var(--bad)', fontSize: 12, fontWeight: 700 }}>your pick</span>}
          </label>
        );
      })}
      {stepResult && step.explanation && (
        <div style={{ marginTop: 4, padding: '10px 12px', borderRadius: 8, background: 'var(--surface-2)', border: '1px solid var(--border)', fontSize: 12.5, color: 'var(--ink-2)', lineHeight: 1.55 }}>
          <strong style={{ color: 'var(--ink)' }}>Why:</strong> {step.explanation}
        </div>
      )}
    </div>
  );
...
function ShortStepBody({ step, answer, setAns, disabled, stepResult }) {
  const text = answer.text || '';
  const words = text.trim().split(/\s+/).filter(Boolean).length;
  return (
    <div>
      {step.guidance && !stepResult && (
        <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8, lineHeight: 1.5 }}>
          <strong style={{ color: 'var(--ink-2)' }}>Aim for:</strong> {step.guidance}
        </div>
      )}
      <textarea
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 6 }}>
        <span className="mono" style={{ fontSize: 10.5, color: words >= (step.min_words || 0) ? 'var(--good)' : 'var(--muted)' }}>{words} words{step.min_words ? ` · aim ~${step.min_words}` : ''}</span>
      </div>
      {stepResult?.feedback && (
        <div style={{ marginTop: 8, padding: '10px 12px', borderRadius: 8, background: 'var(--surface-2)', border: '1px solid var(--border)', fontSize: 12.5, color: 'var(--ink-2)', lineHeight: 1.55 }}>
          <strong style={{ color: 'var(--ink)' }}>Feedback:</strong> {stepResult.feedback}
        </div>
      )}
    </div>
  );
...
function CodeStepBody({ a, step, answer, setAns, disabled, stepResult }) {
  const { add: toast } = useToast();
  const [running, setRunning] = React.useState(false);
  const [runOut, setRunOut] = React.useState(null);
  const code = answer.code ?? step.starter_code ?? '';
  const setCode = (v) => setAns({ code: v });

  const run = async () => {
    setRunning(true); setRunOut(null);
    try { setRunOut(await API.runAssignmentStep(a.id, step.id, code)); }
    catch (e) { toast(e.message || 'Could not run your code', 'error'); }
    finally { setRunning(false); }
  };

  const tests = runOut?.tests;
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <span className="mono" style={{ fontSize: 10.5, padding: '3px 9px', borderRadius: 999, textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700, background: 'color-mix(in oklch, var(--brand-3) 16%, transparent)', color: 'var(--brand-3)', border: '1px solid color-mix(in oklch, var(--brand-3) 35%, transparent)' }}>{STEP_LANG_LABEL[step.language] || step.language}</span>
        <span style={{ fontSize: 12, color: 'var(--muted)' }}>Write it, run it against the sample tests, then submit.</span>
        <span style={{ flex: 1 }} />
        ...
        <div style={{ marginTop: 12 }}>
          {tests && (
            <div style={{ padding: 12, borderRadius: 10, marginBottom: 10, background: 'var(--surface-2)', border: `1px solid ${tests.passed ? 'color-mix(in oklch, var(--good) 40%, var(--border))' : 'color-mix(in oklch, var(--bad) 35%, var(--border))'}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: tests.passed ? 'var(--good)' : 'var(--ink)' }}>{tests.passed ? '✓ Sample tests passed' : `${tests.passedCount}/${tests.total} sample tests passed`}</span>
          ...
          <pre style={{ margin: 0, padding: 12, borderRadius: 10, maxHeight: 220, overflow: 'auto', background: 'oklch(0.16 0.02 270)', color: 'oklch(0.88 0.02 270)', border: '1px solid var(--border)', fontSize: 12, lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
            {runOut.stdout || (runOut.stderr ? '' : '(no output)')}
            {runOut.stderr && <span style={{ color: 'oklch(0.75 0.17 25)' }}>{runOut.stdout ? '\n' : ''}{runOut.stderr}</span>}
          </pre>
          {(runOut.error || runOut.exitCode != null) && (
            <div style={{ fontSize: 11.5, color: runOut.error ? 'var(--bad)' : 'var(--muted)', marginTop: 7 }}>{runOut.error || `Exited with code ${runOut.exitCode}${runOut.durationMs != null ? ` in ${runOut.durationMs} ms` : ''}`}</div>
          )}
        </div>
      )}

      {stepResult && (
        <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 8, background: 'var(--surface-2)', border: `1px solid color-mix(in oklch, ${stepResult.passed ? 'var(--good)' : 'var(--bad)'} 35%, var(--border))`, fontSize: 12.5, color: 'var(--ink-2)' }}>
          <strong style={{ color: 'var(--ink)' }}>Graded:</strong> {stepResult.total ? `${stepResult.passedCount}/${stepResult.total} test cases passed (incl. hidden) · ${stepResult.score}%` : `${stepResult.score}%`}{stepResult.error ? ` · ${stepResult.error}` : ''}
        </div>
      )}
    </div>
  );
